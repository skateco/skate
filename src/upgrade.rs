...
impl<D: UpgradeDeps> Upgrade<D> {
    ...
    async fn upgrade_node(&self, main_args: &UpgradeArgs, args: &NodeArgs) -> Result<(), SkateError> {
        ...
        let node = cluster.nodes.iter().find(|n| n.name == args.name).ok_or("failed to find node".to_string())?;
