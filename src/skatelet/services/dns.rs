...
impl<'a> DnsService<'a>{
    ...
    pub fn add(&self, container_id: String, supplied_ip: Option<String>) -> Result<(), SkateError> {
        ...
        let (extracted_ip, json) = Self::retry(10, || {
            ...
            let output = self.execer.exec("timeout", &["0.2", "podman", "inspect", container_id.as_str()]).map_err(|e| (true, e))?;
            ...
            let output = self.execer.exec("timeout", &["0.2", "podman", "pod", "inspect", pod.unwrap()]).map_err(|e| (true, e))?;
    ...
    pub fn wait_and_enable_healthy(&self,container_id: String) -> Result<(), SkateError> {
        ...
        debug!("{} inspecting container {}",log_tag, container_id);
        let output = self.execer.exec("timeout", &["0.2", "podman", "inspect", container_id.as_str()])?;
        ...
        debug!("{} inspecting pod", log_tag);
        let output = self.execer.exec("timeout", &["0.2", "podman", "pod", "inspect", pod.unwrap()])?;
    ...
    pub fn remove(&self, container_id: Option<String>, pod_id: Option<String>) -> Result<(), SkateError> {
        let tag = {
            if container_id.is_some() {
               container_id.unwrap()
            } else if pod_id.is_some() {
                // get infra container
                let output = self.execer.exec("podman", &["pod", "inspect", &pod_id.unwrap()])?;
                let json: serde_json::Value = serde_json::from_str(&output).map_err(|e| anyhow!("failed to parse podman inspect output: {}", e))?;
    ...
    pub fn reload(&self) -> Result<(), SkateError> {
        let id = self.execer.exec("podman", &["ps", "--filter", "label=skate.io/namespace=skate", "--filter", "label=skate.io/daemonset=coredns", "-q"])?;
        ...
        let _ = self.execer.exec("podman", &["kill", "--signal", "HUP", &id])?;
        Ok(())
    }
}