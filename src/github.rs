...
pub async fn get_latest_release() -> Result<Release, reqwest::Error> {
    let client = reqwest::Client::builder()
        .user_agent(APP_USER_AGENT)
        .build()?;


    client.get("https://api.github.com/repos/skateco/skate/releases/latest")
        .send()
        .await?
        .json::<Release>()
        .await
...
#[cfg(test)]
mod tests {
    use crate::github::get_latest_release;
